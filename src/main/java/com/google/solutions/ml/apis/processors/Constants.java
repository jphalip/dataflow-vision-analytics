...
public interface Constants {
  ...
  List<TableFieldSchema> VERTEX_FIELDS = Arrays.asList(
      new TableFieldSchema().setName(Field.VERTEX_X).setType(BigQueryConstants.Type.FLOAT).setMode(BigQueryConstants.Mode.REQUIRED),
      new TableFieldSchema().setName(Field.VERTEX_Y).setType(BigQueryConstants.Type.FLOAT).setMode(BigQueryConstants.Mode.REQUIRED)
  );

  List<TableFieldSchema> POSITION_FIELDS = Arrays.asList(
      new TableFieldSchema().setName(Field.VERTEX_X).setType(BigQueryConstants.Type.FLOAT).setMode(BigQueryConstants.Mode.REQUIRED),
      new TableFieldSchema().setName(Field.VERTEX_Y).setType(BigQueryConstants.Type.FLOAT).setMode(BigQueryConstants.Mode.REQUIRED),
      new TableFieldSchema().setName(Field.VERTEX_Z).setType(BigQueryConstants.Type.FLOAT).setMode(BigQueryConstants.Mode.NULLABLE)
  );

  List<TableFieldSchema> POLYGON_FIELDS = Collections.singletonList(
      new TableFieldSchema().setName(Field.VERTICES).setType(BigQueryConstants.Type.RECORD).setMode(BigQueryConstants.Mode.REPEATED)
