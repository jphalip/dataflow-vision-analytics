...
public abstract class BatchRequestsTransform extends
    PTransform<PCollection<GCSFileInfo>, PCollection<Iterable<GCSFileInfo>>> {
  ...
  public PCollection<Iterable<GCSFileInfo>> expand(PCollection<GCSFileInfo> input) {
    if (getBatchSize() > 1) {
      return input
          .apply("Assign Keys", WithKeys.of(new SerializableFunction<GCSFileInfo, Integer>() {
            private static final long serialVersionUID = 1L;
      ...
      return input.apply("Convert to Iterable", ParDo.of(new DoFn<GCSFileInfo, Iterable<GCSFileInfo>>() {
        private final static long serialVersionUID = 1L;

        @ProcessElement
        public void process(@Element GCSFileInfo element, OutputReceiver<Iterable<GCSFileInfo>> out) {
